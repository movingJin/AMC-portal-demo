...
const SEL =
  'h-[26px] text-[11px] border border-ink-200 rounded px-1.5 bg-white text-ink-700 focus:outline-none focus:ring-1 focus:ring-brand-300'
...
const IcoBullet = () => (
  <svg viewBox="0 0 24 24" className="w-3.5 h-3.5" fill="currentColor">
    ...
    <rect x="7" y="5" width="13" height="2" />
    <rect x="7" y="11" width="13" height="2" />
    <rect x="7" y="17" width="13" height="2" />
  </svg>
)
const IcoOrdered = () => (
  <svg viewBox="0 0 24 24" className="w-3.5 h-3.5" fill="currentColor">
...
export default function RichTextEditor({ value, onChange }: Props) {
  ...
  useEffect(() => {
    if (editor && value !== editor.getHTML() && !document.activeElement?.closest('.ProseMirror')) {
  ...
  const insertImage = () => {
    const u = prompt('이미지 URL을 입력하세요.')
    if (u) editor.chain().focus().setImage({ src: u }).run()
  }
  const insertYoutube = () => {
    const u = prompt('YouTube URL을 입력하세요.')
  ...
  return (
    <div className="border border-ink-200 rounded-lg">
      ...
      <div className="flex flex-wrap items-center gap-0.5 px-2 py-1.5 bg-ink-50 border-b border-ink-200 sticky top-14 z-20 rounded-t-lg">
        ...
        {/* 글자 크기 */}
        <select
          value={tsAttrs.fontSize?.replace('px', '') ?? ''}
          className={`${SEL} min-w-[76px]`}
          onChange={(e) => {
            const v = e.target.value
        ...
        {/* 줄 간격 */}
        <select
          value={tsAttrs.lineHeight ?? ''}
          className={`${SEL} min-w-[80px]`}
          onChange={(e) => {
            const v = e.target.value
        ...
        <Btn title="텍스트 색상" onClick={() => colorRef.current?.click()}>
          <span
            className="font-bold"
            style={{ color: (editor.getAttributes('textStyle').color as string) ?? '#111' }}
          >
            A
          </span>
        </Btn>
        ...
        >
          <span className="font-bold px-0.5" style={{ background: '#fef08a' }}>
            A
          </span>
        </Btn>
        <input
        ...
        {/* 링크 / 미디어 */}
        <Btn title="링크 삽입" active={editor.isActive('link')} onClick={insertLink}>
          {' '}
          <IcoLink />
        </Btn>
        <Btn
        ...
        <Btn title="이미지 삽입" onClick={insertImage}>
          {' '}
          <IcoImage />
        </Btn>
        <Btn title="YouTube 삽입" onClick={insertYoutube}>
          {' '}
          <IcoYT />
        </Btn>
        ...
        <div className="relative" ref={emojiRef}>
          ...
          {emojiOpen && (
            <div className="absolute top-full left-0 z-50 mt-1 p-2 bg-white border border-ink-200 rounded-lg shadow-lg w-52">
              ...
              <p className="text-[10px] text-ink-400 mt-1.5 px-0.5">
                또는 편집기에서 이모지를 직접 입력
              </p>
            </div>
          )}
        ...
        {/* 표 */}
        <Btn title="표 삽입" onClick={insertTable}>
          {' '}
          <IcoTable />
        </Btn>
        <Btn
        ...
        >
          {' '}
          <span className="text-[10px] font-medium text-red-500">행-</span>
        </Btn>
        <Btn
          title="열 삭제"
          disabled={!inTable}
        ...
        >
          {' '}
          <span className="text-[10px] font-medium text-red-500">열-</span>
        </Btn>
        <Btn
          title="표 삭제"
          disabled={!inTable}
        ...
        >
          {' '}
          <span className="text-[10px] font-medium text-red-500">표✕</span>
        </Btn>
      </div>
