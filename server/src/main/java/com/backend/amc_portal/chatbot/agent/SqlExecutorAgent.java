 ...
 *
 * NOTE: 스펙은 LLM 기반 sub-agent 로 정의되어 있으나 결정론적 로직이라 Java 로 구현하여 LLM 호출 비용을 절감.
 * 분기는 동일:
 *   OK (rows > 0)
 *   RETRY_AUTHOR (UNSAFE_SQL / TIMEOUT / SQL_ERROR / ZERO_ROWS_WITH_VALUE_FILTER)
 *   FAIL (재시도 2회 초과시 메인이 호출)
 *
 * sql-author 재호출은 메인의 책임 — 여기서는 신호만 반환.
 */
@Slf4j
@Component
...
public class SqlExecutorAgent {
    ...
    private static final Pattern VALUE_FILTER_PATTERN = Pattern.compile(
    ...
    public SqlExecutorResult execute(String sql) {
        ...
        List<String> columns = raw.get("columns") instanceof List<?> cl
                ? cl.stream().map(String::valueOf).toList() : List.of();
        @SuppressWarnings("unchecked")
        List<List<Object>> rows = raw.get("rows") instanceof List<?> rl
                ? ((List<List<Object>>) raw.get("rows")) : List.of();
