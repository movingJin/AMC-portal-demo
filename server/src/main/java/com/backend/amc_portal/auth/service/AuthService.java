...
public class AuthService {
    ...
    public void signup(SignupRequest req) {
        ...
        UserStatus initialStatus = requireEmailVerification
    ...
    public void logout(String accessToken, Long userId) {
        try {
            ...
            long ttl = Math.max(0L, (claims.getExpiration().getTime() - System.currentTimeMillis()) / 1000);
    ...
    private TokenResponse issueTokens(User user) {
        ...
        redisTokenService.storeRefreshToken(user.getId(), refresh, tokenProvider.getRefreshTtlSeconds());
        return new TokenResponse(
                access,
                refresh,
                tokenProvider.getAccessTtlSeconds(),
                new TokenResponse.UserSummary(user.getId(), user.getEmail(), user.getDisplayName(), user.getRole().name())
        );
    }
}
